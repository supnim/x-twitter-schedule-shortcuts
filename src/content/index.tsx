 ...
 */
function findStableInsertionPoint(modal: HTMLElement): { parent: HTMLElement; before: HTMLElement | null } | null {
	...
	if (dateGroup) {
		...
		let container = dateGroup.parentElement?.parentElement;
		if (container) {
...
async function mountApp(modal: HTMLElement): Promise<void> {
	...
	try {
		...
		if (isTwitterDarkMode()) {
			rootContainer.classList.add("tss-dark");
		}
...
function repositionRoot(modal: HTMLElement, rootElement: HTMLElement): void {
	try {
		...
		if (rootElement.parentElement === insertionPoint.parent &&
...
function updateTheme(): void {
	...
	if (rootElement) {
		if (currentDarkMode) {
			rootElement.classList.add("tss-dark");
		} else {
			rootElement.classList.remove("tss-dark");
		}
	}
}
		...
		for (const mutation of mutations) {
			...
			if (mutation.type === "childList") {
				...
				for (const node of mutation.addedNodes) {
					if (node instanceof HTMLElement && (node.id === EXTENSION_ROOT_ID || node.querySelector(`#${EXTENSION_ROOT_ID}`))) {
						isOurExtension = true;
						break;
					}
				}
